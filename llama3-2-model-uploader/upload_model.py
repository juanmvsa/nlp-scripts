#!/usr/bin/env python3
"""
main script to upload a fine-tuned llama-3.2 model to hugging face hub.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="upload llama-3.2 finetuned model to hugging face"
    )
    ...
    parser.add_argument(
        "--hf_token", help="huggingface token (or set hf_token env var)"
    )
    parser.add_argument(
        "--private", action="store_true", help="make repository private"
    )
    parser.add_argument(
        "--test", action="store_true", help="test model loading after upload"
    )
    parser.add_argument(
    ...
    if not hf_token:
        print(
            "❌ please provide huggingface token via --hf_token or hf_token env variable"
        )
        return
